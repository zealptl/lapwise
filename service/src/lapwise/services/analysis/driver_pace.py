...
class DriverPaceService:
    ...
    async def _compute_qpace(
    ...
    ) -> tuple[float, Literal["IMPROVING", "DECLINING", "STABLE"], list[Session]]:
        ...
        # Build (score, weight) pairs, most-recent session first (index 0)
        # qual_sessions ordering from get_sessions_for_meetings follows meeting order;
        # reverse so index 0 is most-recent.
        sessions_asc = list(qual_sessions)
        grids_asc = list(grids)
        sessions_desc = list(reversed(sessions_asc))
        grids_desc = list(reversed(grids_asc))

        scores: list[float] = []
        for i, (session, grid) in enumerate(zip(sessions_desc, grids_desc)):
        ...
        n = len(scores)
        mid = n // 2
        # scores[0] is most-recent; first_half = older = scores[mid:]
        # second_half = newer (incl. middle if odd) = scores[:mid] if even, scores[:mid+1] if odd
        if n < 2:
            qpace_trend: Literal["IMPROVING", "DECLINING", "STABLE"] = "STABLE"
        else:
            if n % 2 == 0:
    ...
    async def _compute_sector_deltas(
    ...
    ) -> tuple[float | None, float | None, float | None, Literal["S1", "S2", "S3"] | None]:
        ...
        candidates: list[tuple[float, Literal["S1", "S2", "S3"]]] = []
        ...
        strongest_sector: Literal["S1", "S2", "S3"] | None = (
    ...
    async def _compute_rpace(
        ...
        # Lower rpace = faster; percentile = fraction of other drivers who are slower (higher rpace)
    ...
    async def _compute_all_drivers_rpace(
        ...
        # Fetch stints for each driver in each session
        # Build {driver_number: {session_key: [Stint]}} for lookups
        # We already have all-driver laps; need stints per driver per session
